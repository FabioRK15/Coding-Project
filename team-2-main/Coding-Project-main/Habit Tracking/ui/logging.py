        ...
        for h in state["habits"]:
            if h.name == selected and h.type == "good":
                if h.last_logged == today:
                    ...
                    # 🌸 Spawn flower when good habit is completed today
                    if "flower_status" not in state:
                        state["flower_status"] = [False] * 5

                    for i in range(len(state["flower_status"])):
                        if not state["flower_status"][i]:
                            state["flower_status"][i] = True
                            break

                    # Update flower garden if it exists
