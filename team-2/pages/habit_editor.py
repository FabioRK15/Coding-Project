...
def HabitsView(page: ft.Page):
    ...
    def refresh_habits():
        print(f"DEBUG: refresh_habits() called")  
        good_habits_col.controls.clear()
        bad_habits_col.controls.clear()

        print(f"DEBUG: Getting good habits...")  
        good_habits = db.get_habits_and_icons("Good")
        print(f"DEBUG: Found {len(good_habits)} good habits")  
        
        for h in good_habits:
            row = create_habit_row(h)
            good_habits_col.controls.append(row)

        print(f"DEBUG: Getting bad habits...")  
        bad_habits = db.get_habits_and_icons("Bad")
        print(f"DEBUG: Found {len(bad_habits)} bad habits")  
    ...
    def add_habit(e):
        print(f"DEBUG: Add habit button clicked") 
        ...
        print(f"DEBUG: Name = '{name}', Type = '{habit_type}'")  
        ...
        if name:
            if habit_type == "Bad":
                ...
                if len(bad_habits) >= 5:
                    page.snack_bar = ft.SnackBar(
                        ft.Text("❌ Maximum 5 bad habits allowed (5 mushrooms max)! Delete an existing bad habit first."),
                        bgcolor="#FF5252"
                    )
                    page.snack_bar.open = True
                    page.update()
            ...
            try:
                ...
                print(f"DEBUG: Database call successful")  
            except Exception as ex:
                print(f"ERROR in db.add_habit: {ex}")  
            ...
            print(f"DEBUG: Calling refresh_habits()") 
            refresh_habits()
            print(f"DEBUG: refresh_habits() completed")  
            
            
            success_msg = f"✓ Habit '{name}' added successfully!"
            if habit_type == "Bad":
                success_msg += " (Check dashboard to see the mushroom)"
    ...
    def delete_habit(habit_id):
        ...
        refresh_habits()
        print(f"DEBUG: Habit deleted and UI refreshed")  


    def on_type_change(e):
        print(f"DEBUG: Habit type changed to: {selected_habit_type.current.value}")  
    ...
    def save_edit(e):
        print(f"DEBUG: Saving edit for habit ID: {current_edit_id}")  
    ...
    def open_edit_dialog(h_id, old_name):
        print(f"DEBUG: Opening edit dialog for habit ID: {h_id}")  
    ...
    input_section = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Row(
                ...
                spacing=15),
            ft.Text("Note: Maximum 5 bad habits allowed (5 mushrooms).", 
                   size=12, color="#666666", italic=True),
        ])
    )
    ...
    print(f"DEBUG: HabitsView setup complete, returning view")  
    return ft.View(
        route="/habits",
        bgcolor="#F5F1E8",
